...
# args = c("../../../../output/chr8/chr8-7411171-INV-4101483_6/relate/chr8-7411171-INV-4101483_6.filter.noMissing.haps",  "../../../../output/chr8/chr8-7411171-INV-4101483_6/data/poplabels.relate",  "chr8-7411171-INV-4101483",  "../iqtree_btFASTA/",  "../../../../output/chr8/chr8-7411171-INV-4101483_6/iqtree/chr8-7411171-INV-4101483_6.timetree.nex",  "../../../../output/chr8/chr8-7411171-INV-4101483_6/iqtree_btFASTA/all.minMutHomoplasyBT.dat",  "chr8-7411171-INV-4101483_6")
...
list_trees_start <- as.numeric(unlist(lapply(strsplit(basename(list_trees), split="\\."), function(x) x[2])))
...
for(f in df_list_trees$path){
  tmpID <- unlist(strsplit(basename(f), split = "\\."))[2]
...
if (num_trees == 0){
  ggsave(paste(outf_prefix,".densitree.btTrees.pdf",sep=''), height = 9, width = 12, limitsize=F,useDingbats=FALSE)
  
  ggsave(file=glue("{outf_prefix}.TreeSimilarity.btTrees.pdf"),
       height=6, 
       width = 9, useDingbats=FALSE)
  file.create(paste(outf_prefix,"df.homoplasyBTtrees.dat", sep = "."))

  print (paste0("ZERO BT trees available. Empty plots generated!"))
  q()
}
...
write.table(df_homoplasy, paste(outf_prefix,"df.homoplasyBTtrees.dat", sep = "."))
...
p_densi <- ggdensitree(new_trees, alpha=0.3, tip.order = 1, layout = "rectangular", aes(colour=colID, size=colID)) %<+% df_tree +  scale_size_manual( values = c(2,1) ) +
  ...
  guides(linetype="none", size="none", 
         ...
         shape=guide_legend(nrow=1, title=paste(outf_prefix, sprintf("#Bootstrap trees:%s",num_trees),sep="\n"))) + 
...
#    scale_fill_brewer(palette = "Spectral", direction = -1) + 
    ...
    ggtitle(paste(var_ID," (",numWindows," bootstrap trees)",sep=""))
}
...
ggsave(plot=plt,file=glue("{outf_prefix}.TreeSimilarity.btTrees.pdf"),
#       height=12*scale,
       height=6, 
       width = 9, useDingbats=FALSE
)
