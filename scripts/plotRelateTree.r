...
#args = c("output/chr19/chr19-47959661-INV-2131536_1/relate/chr19-47959661-INV-2131536_1.filter.noMissing.haps", "output/chr19/chr19-47959661-INV-2131536_1/data/poplabels.relate" ,"chr19-47959661-INV-2131536", "output/chr19/chr19-47959661-INV-2131536_1/relate/all.nwk", "output/chr19/chr19-47959661-INV-2131536_1//iqtree/chr19-47959661-INV-2131536_1.timetree.nex", "output/chr19/chr19-47959661-INV-2131536_1/relate/all.minMutHomoplasy_phylo.dat", "chr19-47959661-INV-2131536_1")


#args = c("../../../../output/chrX/chrX-51667498-INV-58316/relate/chrX-51667498-INV-58316.filter.noMissing.anc.mask.haps", "../../../../output/chrX/chrX-51667498-INV-58316/data/poplabels.relate","chrX-51667498-INV-58316",  "../../../../output/chrX/chrX-51667498-INV-58316/relate/all.nwk", "../../../../output/chrX/chrX-51667498-INV-58316/iqtree/chrX-51667498-INV-58316.timetree.nex", "../../../../output/chrX/chrX-51667498-INV-58316/relate/all.minMutHomoplasy_phylo.dat", "chrX-51667498-INV-58316")
...
# prepare and merge all newick trees (timetree + relate trees)
mytimetree2 <- list()
mytimetree2[[outf_prefix]] <- treeio::drop.tip(read.nexus(fin_timetree), "CMP")
mytimetree2[[outf_prefix]]$edge.length <- mytimetree2[[outf_prefix]]$edge.length
...
if (class(tree) != "phylo"){
  for (i in 1:length(tree)){
    mytimetree[[as.character(i-1)]] <- treeio::drop.tip(tree[[i]], "CMP")
    mytimetree[[as.character(i-1)]]$edge.length <- mytimetree[[as.character(i-1)]]$edge.length * 29 / 1e6
  }
}else{
  mytimetree[[as.character(0)]] <- treeio::drop.tip(tree, "CMP")
  mytimetree[[as.character(0)]]$edge.length <- mytimetree[[as.character(0)]]$edge.length * 29 / 1e6
}
...
# Note that the branch length of a Relate tree is in generation, while that of a phylo tree is in years
sumBranchLength <- function(x, mPhyloObj, inType = "phylo"){
  yrs_gen <- 29
  if (inType == "phylo"){
    return(sum(mPhyloObj[[x]]$edge.length * 1e6 / yrs_gen))
  }else if(inType == "relate"){
    return(sum(mPhyloObj[[x]]$edge.length * 1e6 / yrs_gen))
  }
} 
...
df_homoplasy$totalBranchLength <- apply(df_homoplasy, 1, function(x) ifelse(x[1]==outf_prefix, sumBranchLength(x[1], mytimetree2, "phylo"), sumBranchLength(x[1], mytimetree2, "relate")))

df_homoplasy$invPerGeneration <- df_homoplasy$V2/df_homoplasy$totalBranchLength


write.table(df_homoplasy, paste(outf_prefix,"df.homoplasyRelateTrees.dat", sep = "."))
...
p_densi <- ggdensitree(new_trees, alpha=0.3, tip.order = 1, layout = "rectangular", aes(colour=colID, size=colID)) %<+% df_tree +  scale_size_manual( values = c(2,1) ) +
  ...
  guides(linetype="none", size="none", 
         ...
         shape=guide_legend(nrow=1, title=paste(outf_prefix, sprintf("#Local trees [Relate]:%s",num_trees),sep="\n"))) + 
...
# basic stats
pointEst_numINVs <- df_homoplasy[ df_homoplasy$V1 == outf_prefix, "V2"]
pointEst_invPerGen <- df_homoplasy[ df_homoplasy$V1 == outf_prefix, "invPerGeneration"]
median_numINVs <- median(df_homoplasy$V2)
CI_numINVs <- quantile(df_homoplasy$V2, probs = c(0.025,0.975))
median_invPerGen <- median(df_homoplasy$invPerGeneration)
...
p_lone = make_tri(sdf, var_ID=outf_prefix, listColors = mycolors_tri)
scale = 1/2.25
...
ymin = yr[2] * 1/1.8
ymax = yr[2] * 2/2
print(paste(outf_prefix, xmin, xmax, ymin, ymax))
# combine
