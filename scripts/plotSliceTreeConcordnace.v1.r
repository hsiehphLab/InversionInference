...
#args = c("output/chr11/chr11-50136371-INV-206505/relate/chr11-50136371-INV-206505.filter.haps", "output/chr11/chr11-50136371-INV-206505/relate/chr11-50136371-INV-206505.filter.poplabels", "chr11-50136371-INV-206505", "output/chr11/chr11-50136371-INV-206505/iqtree_sliceFASTA/all.sliceTrees", "tmp")
...
mytimetree <- lapply(tree, function(x) treeio::drop.tip(x, "CMP"))
class(mytimetree) <- "multiPhylo"
#mytimetree2 <- tree
mytimetree2 <- lapply(mytimetree, function(x) force.ultrametric(x))
...
trees[[length(mytimetree2)]]$colID <- "a"
...
#  scale_x_continuous(limits = c(upXlim,-lowXlim), breaks=list_breaks, labels=-list_breaks) + 
  guides(linetype="none", size="none", color=guide_legend(title="Population",nrow=2), shape=guide_legend(nrow=2,title=paste("Inversion",outf_prefix, sprintf("#Local trees:%s",num_trees),sep="\n"))) + theme(legend.position="top", legend.text = element_text(size=16), legend.title = element_text(size=16), legend.key.size = unit(1, 'cm'), axis.text.x = element_blank() ) 

ggsave(paste(outf_prefix,".densitree.sliceTrees.pdf",sep=''), height = 9, width = 12, limitsize=F)
...
#    scale_fill_brewer(palette = "Spectral", direction = -1) + 
    ...
    ggtitle(paste(var_ID," (",numWindows," windows)",sep=""))
}
...
mat_mutualClustering <- as.matrix(MatchingSplitInfo(tree, normalize = T))
