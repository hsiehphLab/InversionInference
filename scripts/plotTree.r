...
#args = c("output/chr8/chr8-7411171-INV-4101483_1/data/chr8-7411171-INV-4101483_1.filter.noMissing.vcf.hap.tped",  "output/chr8/chr8-7411171-INV-4101483_1/data/chr8-7411171-INV-4101483_1.filter.noMissing.vcf.hap.tfam", "output/chr8/chr8-7411171-INV-4101483_1/data/IDs_wPopID.hapID",  "output/chr8/chr8-7411171-INV-4101483_1/iqtree/chr8-7411171-INV-4101483_1.treefile",  "output/chr8/chr8-7411171-INV-4101483_1/iqtree/chr8-7411171-INV-4101483_1.timetree.nex",  "chr8-7411171-INV-4101483",  "chr8-7411171-INV-4101483_1")
...
orig1_tped <- as.data.frame(data.table::fread(fin_tped, header = FALSE, sep = " "))
orig_tped <-  t(orig1_tped[,-c(1,2,3,4)])
orig_tfam <- read.delim(fin_tfam, header = FALSE, sep = "")
#orig_tfam$V7 <- apply(orig_tfam, 1, function(x) paste(unlist(strsplit(as.character(x),"_"))[2:3],collapse="_"))

orig_tfam$V7 <- sapply(strsplit(as.character(orig_tfam$V1), "_"), function(x) paste(x[2:3], collapse = "_"))
...
rt_iqtree2 <- ape::root(iqtree2@phylo, outgroup = "CMP", resolve.root = TRUE)
rt_iqtree2$edge.length <- rt_iqtree2$edge.length / max(rt_iqtree2$edge.length)
numZeroBL = table(rt_iqtree2$edge.length == 0)[2]
numZeroBL = ifelse(is.na(numZeroBL), 0, numZeroBL)
print(iqtree2@phylo)
print(numZeroBL)
print (Ntip(rt_iqtree2))
if (numZeroBL) {
...
#  rt_iqtree2$edge.length = rt_iqtree2$edge.length + 1e-8
  rt_iqtree2$edge.length[rt_iqtree2$edge.length <= 0] <- 1e-8
}
my_node <- getMRCA(rt_iqtree2, tip=c("CMP", rt_iqtree2$tip.label[2])) 
mycalibration <- makeChronosCalib(phy = rt_iqtree2, node = my_node, age.min = 5.5, age.max = 6 )

N_OUTER   <- 1    # number of independent convergence runs to collect
MAX_INNER <- 5    # hard cap per outer run (was unlimited for i==1)
MAX_TOTAL <- 10   # absolute total call limit across all runs
# Relaxed but still tight tolerances:
CHRONOS_CTRL <- chronos.control(
  epsilon        = 1e-4,   # was 1e-8
  eval.max       = 1e6,    # was 1e5
  iter.max       = 150,
  dual.iter.max  = 100,    # was 1000
  tol            = 1e-4,   # was 1e-8
  nb.rate.cat    = 1
)

count_eval      <- 0
list_mytimetree <- vector(mode = "list", length = N_OUTER)
...
#    cat(sprintf("  Outer run %d / inner iter %d (total calls: %d)\n",
#                i, inner_count, count_eval))

    result <- tryCatch(
      chronos(rt_iqtree2, lambda = 10,
              calibration = mycalibration,
              model = "discrete",
              control = CHRONOS_CTRL),
      error = function(e) { cat("  chronos error:", conditionMessage(e), "\n"); e }
    )

    if (inherits(result, "error")) {
      if (inner_count >= MAX_INNER || count_eval >= MAX_TOTAL) break
      next
    }

    convergence <- isTRUE(attr(result, "convergence"))
    cat("  convergence =", convergence, "\n")
	cat("  message =", attr(result, "message"))

    if (convergence) {
      list_mytimetree[[i]] <- result
      break
    }

    # Hard caps to prevent infinite spinning
    if (inner_count >= MAX_INNER) {
      cat(sprintf("  Reached MAX_INNER (%d) for run %d moving on.\n",
                  MAX_INNER, i))
      break
    }
    if (count_eval >= MAX_TOTAL) {
      cat(sprintf("  Reached MAX_TOTAL (%d) stopping chronos.\n", MAX_TOTAL))
      break
    }
  }

  if (count_eval >= MAX_TOTAL) break
}

# Keep only successful (non-NULL) runs
list_mytimetree <- Filter(Negate(is.null), list_mytimetree)
# Also filter for runs where convergence attribute is TRUE
list_mytimetree <- Filter(function(x) isTRUE(attr(x, "convergence")), list_mytimetree)
 
if (length(list_mytimetree) == 0) {
  cat("WARNING: chronos did not converge in any run.\n",
      "Using the last non-error result as a best-effort estimate.\n",
      "Consider increasing MAX_INNER/MAX_TOTAL or loosening tolerances further.\n")
  # Fallback: just keep the last non-error result regardless of convergence
  last_result <- tryCatch(
    chronos(rt_iqtree2, lambda = 10, calibration = mycalibration,
            model = "discrete",
            control = chronos.control(epsilon = 1e-4, eval.max = 1e6,
                                      dual.iter.max = 100, tol = 1e-4,
                                      nb.rate.cat = 1)),
    error = function(e) NULL
  )
  if (!is.null(last_result)) list_mytimetree <- list(last_result)
}
 
if (length(list_mytimetree) == 0) stop("chronos failed entirely. check your tree and calibration.")
 
list_logLik <- sapply(list_mytimetree, function(x) attr(x, "PHII")$logLik)
mytimetree_ape <- list_mytimetree[[which.max(list_logLik)]]
 

## plot dated (APE) tree 
write.tree(mytimetree_ape, paste0(outf_prefix, ".mytimetree.nwk"))
mytimetree_ape_tr <- read.iqtree(paste0(outf_prefix, ".mytimetree.nwk"))

num_seqs <- length(mytimetree_ape_tr@phylo$tip.label)
df_mytimetree <- as.data.frame(as_tibble(mytimetree_ape_tr))
...
df_mytimetree$hapID <- mapping_hap_INV$hapID[ match(df_mytimetree$label, mapping_hap_INV$orig_hapID)]
...
p1 <- ggtree(mytimetree, aes(color=Pop, linetype=branchGroup), size=1.2)  %<+% df_mytimetree + 
  ...
  scale_color_manual(values=cols, breaks=c("AFR","AMR","EAS","EUR","SAS")) + 
 # geom_tiplab(aes(label=mapping_hap_INV[ match(df_mytimetree$label, mapping_hap_INV$orig_hapID),"hapID"], subset=INV), linetype=NA, size=5, hjust=-0.1) +  
  geom_tiplab(aes(label = hapID, subset = INV), linetype = NA, size = 5, hjust = -0.1) +
  guides(linetype="none", size="none", color=guide_legend(title="Population", nrow=1), shape=guide_legend(title=paste("Inversion",outf_prefix,sep="\n"), nrow=1)) + theme_tree2() + 
  theme(legend.position="top", legend.justification='left', legend.text = element_text(size=14), legend.title = element_text(size=12), legend.key.size = unit(1, 'cm'), axis.text.x = element_text(size=14)) + xlab("Million years") 
